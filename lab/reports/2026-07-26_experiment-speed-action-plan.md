...
| PERF-DEV-17 | 등록된 repeat 계약의 "same reserved GPU" 절반 | **미이행.** emitter는 r1..r3를 내지만 세 repeat가 같은 예약 GPU에 떨어지도록 강제하지 않는다. 현 queue worker는 GPU를 소유권으로 배정하므로, P0.0 예약 시 이 조건을 운영 절차로 보장해야 한다. |
